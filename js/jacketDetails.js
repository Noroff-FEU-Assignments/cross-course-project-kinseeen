...
async function jacketInformation () {
    try {
        const response = await fetch (`https://rainydays.kinejakobsen.tech/wp-json/wc/v3/products/${id}?consumer_key=ck_3c0d30cdff7e3750655598c1be8392865f8bf3f8&consumer_secret=cs_8aac1bee83badcf77a5c6b8cf5ef1b5914ab92ad`)
        const reposonseAsJson = response.json();
        console.log(reposonseAsJson);
        detailsContainer.innerHTML = "";
        setTitle(reposonseAsJson.name);
        for (let i = 0; i < reposonseAsJson.length; i++) {
            console.log(reposonseAsJson[i]);
